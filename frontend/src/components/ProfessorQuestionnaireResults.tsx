...
function ProfessorQuestionnaireResultsComponent({
  course,
  professor,
}: ProfessorQuestionnaireResultsProps) {
  const [results, setResults] = useState<ProfessorQuestionnaireResults | null>(
    null,
  );
  ...
  const fetchResults = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/fetchCAP/course/${course._id}/professor/${professor._id}`,
      );
      if (!response.ok) {
        setResults(null);
        return;
  ...
  const runSearch = useCallback(
    async (query: string) => {
      ...
      try {
        const response = await fetch(
          `/api/searchCAP/search?query=${encodeURIComponent(query.trim())}&courseId=${course._id}&professorId=${professor._id}`,
        );
        if (!response.ok) {
          setSearchResults([]);
          return;
